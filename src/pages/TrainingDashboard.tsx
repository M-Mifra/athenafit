...
import { ArrowLeft, Send, Loader2, History, RefreshCw, BookOpen, Activity, Sparkles, TrendingUp, Calendar } from "lucide-react";
...
const TrainingDashboard = () => {
  ...
  const handleReset = () => {
    ...
    setBioData({ sleep: 4, energy: 3, stress: 3, soreness: 2, timeAvailable: 3 });
    setEnvData({ aqi: 50, temperature_celsius: 22, is_heatwave: false, lockdown_status: "none", has_local_event: false });
      ...
      <header className="sticky top-0 z-50 bg-background/80 backdrop-blur-md border-b border-border/50">
        <div className="container flex h-16 items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <Button variant="ghost" size="sm" onClick={() => navigate("/methodology")} className="gap-2">
          ...
          <div className="grid lg:grid-cols-12 gap-8 items-start">
            <div className="lg:col-span-7 space-y-8">
              ...
              <Button
                ...
                disabled={loading}
                className="w-full h-16 text-lg font-bold bg-primary hover:scale-[1.01] transition-all rounded-2xl shadow-xl shadow-primary/20 border-t border-white/20"
              >
                {loading ? <Loader2 className="h-6 w-6 animate-spin" /> : <><Send className="h-5 w-5 mr-3" /> Generate Training Strategy</>}
              </Button>
            ...
            <div className="lg:col-span-5 sticky top-24 space-y-6">
              {result ? (
                ...
                <div className="space-y-6">
                  <div className={`p-5 rounded-2xl border ${getDecisionBg(livePreview.final_decision)} shadow-sm`}>
                    <div className="flex items-center justify-between mb-4">
                      ...
                      <div className="text-right">
                        ...
                        <p className="font-mono text-3xl font-black">{livePreview.readiness_score}</p>
                      </div>
                    </div>
